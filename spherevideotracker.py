...
class spherevideotracker():
    def __init__ (self,videosource, width=640, height=480) :
        ...
        self.movCumEjeX = 0
        self.movCumEjeY = 0
        self.WORKING_MIN_CONTOUR_AREA = 0 #needs calibrate
        self.WORKING_MAX_CONTOUR_AREA = 0 #needs calibrate
...
def startcapture(tracker):
    ...
    #Nombre: Movement Indicator
    cv2.namedWindow(tracker.winName, cv2.CV_WINDOW_AUTOSIZE)
    

    if tracker.calibrated == False :
        tracker.calibrate()

    # Se declaran unas imágenes, para inicializar correctamente cámara y variables.
    t_current = cv2.cvtColor(tracker.cam.read()[1], cv2.COLOR_RGB2GRAY)
    t_plus = cv2.cvtColor(tracker.cam.read()[1], cv2.COLOR_RGB2GRAY)
    ...
    while True:
        ...
        for i in range(0,len(contours)):
            ...
            center = (int(x),int(y))
            radius = int(radius)
            if cv2.contourArea(cnt) > tracker.WORKING_MIN_CONTOUR_AREA and cv2.contourArea(cnt) < tracker.WORKING_MAX_CONTOUR_AREA: 
                #áreas muy chicas pueden significar ruido que se mueve, mejor ignorarlo..
                cv2.circle(im,center,radius,(0,255,0),2)
                L.append(center)
        ...
        for index in range(len(Lnuevo)):
            for j in range(index, len(L)):
                if (math.sqrt((Lnuevo[index][0] - L[j][0]) ** 2 + (Lnuevo[index][1] - L[j][1]) **
                    ...
                    numberOfVectors+=1
                    if (Lnuevo[index][1] > tracker.height / 2):
        ...
        tracker.movEjeX = movEjeX
        tracker.movEjeY = movEjeY
        tracker.movCumEjeX += movEjeX
        tracker.movCumEjeY += movEjeY
        ...
        cv2.imshow( tracker.winName , im ) #obs.: NO es estrictamente necesario dar feedback acá.. también está mainFunction
        ...
        if (key == 27 or key==1048603): #escape pressed
            #end Program.
            cv2.destroyWindow(tracker.winName)
...
if __name__ == '__main__':
    ...
    sp.calibrate()
    import threading
    fread = threading.Thread(target=startcapture(sp))
    time.sleep(2)
    print "a"
    fread.start()
